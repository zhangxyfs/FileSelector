...
public class CheckView extends View {
    ...
    private static int SIZE = 30; // dp
    private static float STROKE_RADIUS = 11.5f; // dp
    private static float BG_RADIUS = 11.0f; // dp
    private static float STROKE_WIDTH = 3.0f; // dp
    private static String shaderStart = "#00000000";
    private static String shaderEnd = "#0D000000";
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultColor = ResourcesCompat.getColor(
                getResources(), R.color.box_item_checkCircle_borderColor,
                getContext().getTheme());
        int color = ta.getColor(0, defaultColor);
        ta.recycle();
        mStrokePaint.setColor(color);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.boxCheck);
        shaderStart = a.getString(R.styleable.boxCheck_shaderStart);
        shaderEnd = a.getString(R.styleable.boxCheck_shaderEnd);
        if (TextUtils.isEmpty(shaderStart)) {
            shaderStart = "#00000000";
        }
        if (TextUtils.isEmpty(shaderEnd)) {
            shaderEnd = "#0D000000";
        }
        int circleColor = a.getColor(R.styleable.boxCheck_circleColor, defaultColor);
        SIZE = a.getInt(R.styleable.boxCheck_sizeDP, 30);
        STROKE_RADIUS = a.getFloat(R.styleable.boxCheck_strokeRadius, 11.5f);
        BG_RADIUS = a.getFloat(R.styleable.boxCheck_bgRadius, 11.0f);
        STROKE_WIDTH = a.getFloat(R.styleable.boxCheck_zStrokeWidth, 3.0f);
        ...
        mCheckDrawable = ResourcesCompat.getDrawable(context.getResources(),
                R.drawable.ic_file_check_white_18dp, context.getTheme());
    }
        ...
        if (mShadowPaint == null) {
            ...
            mShadowPaint.setShader(
                    new RadialGradient((float) SIZE * mDensity / 2,
                            ...
                            new int[]{Color.parseColor(shaderStart), Color.parseColor(shaderEnd),
                                    Color.parseColor(shaderEnd), Color.parseColor(shaderStart)},
    ...
    private void initBackgroundPaint() {
        if (mBackgroundPaint == null) {
            ...
            int defaultColor = ResourcesCompat.getColor(
                    getResources(), R.color.box_item_checkCircle_backgroundColor,
                    getContext().getTheme());
            int color = ta.getColor(0, defaultColor);
            ta.recycle();
